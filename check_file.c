...
/*   Updated: 2020/10/20 20:11:14 by jheat            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub.h"

static int		is_ident_true(t_all *all, char li)
{
	if (li == '\0')
		return (0);
	if (all->no_t && all->so_t && all->we_t && all->ea_t
		&& all->s_t && all->floor_col >= 0 && all->ceiling_col >= 0
		&& all->win_h >= 0 && all->win_w >= 0)
		return (1);
	return (0);
}
...
static int		get_f_and_c_col(char *s)
{
	...
	if (r > 255 || g > 255 || b > 255 || r < 0 || g < 0 || b < 0)
		exit_err("Not valid Floor Or Ceil color", 22);
	free(line);
...
void			check_line(t_all *all, char *s)
{
	...
	else if (is_ident_true(all, *li) && !(*li == 'C' && *(li + 1) == ' '))
